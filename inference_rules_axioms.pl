...
% EQ: right expansion
inference_rule_r(entails(Γ, [_, transition(X, B, Y)]), [entails([u : A], [u : B]), entails([u : B], [u : A])], 'EQ') :-
...
% ATM
inference_rule_l(entails([Γ, X : P], Δ), [entails([X : P, Y : P | Γ], Δ)], 'ATM') :-
    (
        member(Y >= X, Γ), !;
        X = Y
    ),
...
% MON
inference_rule_l(entails([Γ, transition(X, A and B, Y)], Δ), [entails([transition(X, A and B, Y), transition(X, A, Y), transition(X, B, Y) | Γ], Δ)], 'MON') :-
    \+member(transition(X, A, Y), Γ),
    \+member(transition(X, B, Y), Γ).

% CA
inference_rule_l(entails([Γ, transition(X, A or B, Y)], Δ), [entails([transition(X, A or B, Y), transition(X, A, Y) | Γ], Δ), entails([transition(X, A or B, Y), transition(X, B, Y) | Γ], Δ)], 'CA') :-
    \+member(transition(X, A, Y), Γ),
    \+member(transition(X, B, Y), Γ).

% Unit
inference_rule_l(entails([Γ, transition(X, A, Y)], Δ), [entails([Y >= X, transition(X, A, Y) | Γ], Δ)], 'Unit') :-
    \+member(Y >= X, Γ).

% ID
inference_rule_l(entails([Γ, transition(X, A, Y)], Δ), [entails([transition(X, A, Y), Y : A | Γ], Δ)], 'ID') :-
    \+member(Y : A, Γ).

% CA - conv
inference_rule_l(entails([Γ, transition(X, A, Y)], Δ), [entails([transition(X, A or B, Y), transition(X, A, Y) | Γ], Δ)], 'CA - conv') :-
    member(A or B says _, Γ),
    \+member(transition(X, A or B, Y), Γ).

% DT
inference_rule_l(entails([Γ, transition(X, A, Y)], Δ), [entails([transition(X, A, Y) | Γ], [Y : B | Δ]), entails([transition(X, A, Y), transition(X, A and B, Y) | Γ], Δ)], 'DT') :-
    member(A and B says _, Γ),
    \+member(transition(X, A and B, Y), Γ),
    \+member(Y : B, Δ).

% EQ: left expansion
inference_rule_l(entails([_, transition(X, A, Y)], Δ), [entails([u : A], [u : B]), entails([u : B], [u : A])], 'EQ') :-
    member(transition(X, B, Y), Δ).

% Trans
inference_rule_l(entails([Γ, E], Δ), [entails([Z >= X, E | Γ], Δ)], 'Trans') :-
    member(Z >= Y, [E | Γ]),
    member(Y >= X, [E | Γ]),
    \+member(Z >= X, [E | Γ]).

% C
inference_rule_l(entails([Γ, E], Δ), [entails([transition(Z, A, Z), E | Γ], Δ)], 'C') :-
    (
        member(Z >= Y, [E | Γ]), !;
        Y = Z
    ),
...
% VD
inference_rule_l(entails([Γ, E], Δ), [entails([transition(X, A, Z), E | Γ], Δ)], 'VD') :-
    member(Y >= X, [E | Γ]),
